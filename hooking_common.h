...
// Function to convert WCHAR* to const char*
const char* ConvertWCHARToChar(const WCHAR * wStr) {
	// Determine the required buffer size for the multibyte string
	size_t bufferSize = wcstombs(nullptr, wStr, 0) + 1; // +1 for null terminator

	// Allocate memory for the multibyte string
	char* mbs = static_cast<char*>(malloc(bufferSize));
	if (mbs == nullptr) {
		return nullptr;
	}

	// Perform the conversion
	wcstombs(mbs, wStr, bufferSize);

	return mbs;  // The caller is responsible for freeing this memory
}
...
void RebaseVirtualDrivePath(const char* path, char* outBuff, size_t outBuffSize)
{
	...
	QueryDosDeviceA(driveLetter, deviceDrive, 512);
...
HMODULE FindModuleInProcess(HANDLE process, const char* name)
{
	...
	GetModuleFileNameExA(process, NULL, remoteProcessName, 256); //a null module handle gets the process name
	...
	for (DWORD i = 0; i < numRemoteModules; ++i)
	{
		...
		GetModuleFileNameExA(process, remoteProcessModules[i], moduleName, 256);
...
DWORD FindPidByName(const char* name)
{
	...
	do {
		const char* str = ConvertWCHARToChar(singleProcess.szExeFile);
	...
	if (EnumProcessModules(process, hMods, sizeof(hMods), &cbNeeded))
	{
		for (uint32_t i = 0; i < (cbNeeded / sizeof(HMODULE)); i++)
		{
				...
				sizeof(moduleName) / sizeof(TCHAR)))
			{
				...
				if (strstr(reinterpret_cast<const char*>(moduleName), targetModule) != nullptr)
				{
					return hMods[i];
				}
			}
...
void* FindAddressOfRemoteDLLFunction(HANDLE process, const char* dllName, const char* funcName)
{
	...
	HMODULE localDLL = LoadLibraryExA(dllName, NULL, 0);
	check(localDLL);
